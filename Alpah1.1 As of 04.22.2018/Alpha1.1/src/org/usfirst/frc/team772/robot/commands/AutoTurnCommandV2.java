 ...
 *
 */
public class AutoTurnCommandV2 extends Command {
	 ...
	 */
	public AutoTurnCommandV2(double degreesNeededToTurn) {
	...
	protected void execute() {
		...
		double minspeed = RobotMap.AUTO_TURN_MIN_SPEEDV2;
		double minangle = 0.1;
		double maxspeed = RobotMap.AUTO_TURN_MAX_SPEEDV2;
		double maxangle = 60;
		// the plan:
	...
	protected boolean isFinished() {
		if ((Math.abs(degreesLeftToTurn) <= RobotMap.AUTO_TURN_TOLERANCEV2) || timer.hasPeriodPassed(3)) {//1 sec
			return true;
		} else {
			return false;
		}
	...
	protected void end() {
		...
		SmartDashboard.putString("DB/String 8", "gyro: " + Robot.driveSubsystem.getDegrees());
		SmartDashboard.putString("DB/String 2", "rotation: " + Math.round(rotationSpeed * 100) / 100);
		SmartDashboard.putString("DB/String 5", "DegNeeded: " + degreesNeededToTurn);
		SmartDashboard.putString("DB/String 0", "DegLeft: " + degreesLeftToTurn);
		SmartDashboard.putString("DB/String 4", "Rotation: " + rotationSpeed);
